...
extension WDController:  UIImagePickerControllerDelegate, UINavigationControllerDelegate{
...
class WDController:UIViewController, UITextViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        if(preferences.string(forKey: "titleCache") != nil){
            ...
            titleInput.text = preferences.string(forKey: "titleCache")
            diaryInput.text = preferences.string(forKey: "diaryCache")
            textWatcher.text = String(diaryInput.text.count) + " / 480"
            
            if(preferences.string(forKey: "imageCache") != nil){
                attachPictureImage.setImage(UIImage(named: "wd_asset_picture_attached"), for: .normal)
            }
            
            setPalett(parameter : preferences.integer(forKey: "backgroundCache"))
    ...
    func startDiaryAction(title:String){
        ...
        let fileUrl = documentDirectoryUrl.appendingPathComponent("\(preferences.string(forKey: "currentDate")!).json")
